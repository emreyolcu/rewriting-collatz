...
def search(solver, timeout, system, interpretation, dimension, resultwidth, phasesaving, workers):
    phase = '-phs save' if phasesaving else '-phs negative'
    work = f'-work {workers}' if workers > 1 else f'-work 1 -noshuf'
    cmd = f'python prover/main.py {system} -i {interpretation} -d {dimension} -rw {resultwidth} -any -once {phase} {work} -sol {solver}'
...
def main():
    ...
    experiments = [
        ...
        ('relative/collatz-T-08.srs', 'natural', 4, 4),
        ('relative/collatz-T-11.srs', 'natural', 4, 4),
        ('relative/collatz-T-11.srs', 'arctic', 4, 3),
        ('relative/collatz-C-6mod8-2.srs', 'natural', 3, 11),
    ...
    for e in experiments:
        ...
        for phase in (True, False):
            for workers in (1, 8):
                results.clear()
                stmt = f"results.append(search('{solver}', {timeout}, '{instance}', '{interpretation}', {dimension}, {resultwidth}, {phase}, {workers}))"
